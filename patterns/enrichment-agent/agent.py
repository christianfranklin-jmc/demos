"""AgentCore Runtime entry point for the Enrichment Agent (FAST pattern).
...
The Enrichment Agent generates RAG-enhanced descriptions, applies DataZone
predictions, enriches Glue Data Catalog metadata, and produces semantic
layer YAML files.
...
async def invocations(payload: dict, context: RequestContext):
    ...
    # Auto-connect to catalog if env vars are set
    db_host = os.environ.get("DB_HOST")
    if db_host and "connected" not in prompt.lower():
        driver_type = os.environ.get("DB_DRIVER_TYPE", "postgresql")
        db_name = os.environ.get("DB_NAME", "db")
        prompt = (
            f"The target catalog is auto-connected: driver_type={driver_type}, "
            f"database={db_name}. Use scan_metadata to discover tables needing "
            f"enrichment, then proceed: {prompt}"
        )
