"""AgentCore Runtime entry point for the Quality Agent (FAST pattern).
...
The Quality Agent generates DQDL rules, executes quality evaluations,
manages quarantine workflows, generates dbt tests, and proposes
...
async def invocations(payload: dict, context: RequestContext):
    ...
    # Auto-connect to DB if env vars are set
    ...
    if db_host and "connected" not in prompt.lower():
        ...
        prompt = (
            ...
            f"database={db_name}. Use scan_metadata and profile_database to "
            f"assess data quality, then proceed: {prompt}"
        )
