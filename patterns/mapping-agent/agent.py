"""AgentCore Runtime entry point for the Mapping Agent (FAST pattern).
...
The Mapping Agent extracts entities and relationships, generates RDF
triples, loads graph data into Neptune, and produces dbt lineage
documentation.
...
async def invocations(payload: dict, context: RequestContext):
    ...
    if db_host and "connected" not in prompt.lower():
        ...
        prompt = (
            ...
            f"and relationships, then proceed: {prompt}"
        )
