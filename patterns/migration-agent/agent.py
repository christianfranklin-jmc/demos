"""AgentCore Runtime entry point for the Migration Agent (FAST pattern).
...
The Migration Agent extracts schemas, data, and semantic metadata from
Snowflake, converts to Iceberg, exports to S3, registers in Glue Catalog,
and auto-scaffolds a dbt project for the migrated warehouse.
...
async def invocations(payload: dict, context: RequestContext):
    ...
    # Auto-connect to source DB if env vars are set
    sf_account = os.environ.get("SF_ACCOUNT")
    if sf_account and "connected" not in prompt.lower():
        prompt = (
            f"The Snowflake source is configured: account={sf_account}. "
            f"Use connect_to_database with driver_type=snowflake to connect, "
            f"then scan_metadata to discover the schema. Task: {prompt}"
        )
