...
- `model_agent`, `quality_agent` — still stubs that emit realistic-looking artifacts (dbt model paths, DQDL rule sets). Real bodies land when the live demo path needs them; the `pyiceberg`+`dbt-glue` write loop is exercised end-to-end via `mapping_agent` + `pipeline_agent` already.
