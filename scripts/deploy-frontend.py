...
def parse_config_yaml(config_path: Path) -> Dict[str, str]:
    """
    Parse config file using regex (supports YAML and Terraform tfvars).

    Args:
        config_path: Path to config file (config.yaml or terraform.tfvars)
    ...
    # Support both YAML (key: value) and HCL (key = "value") formats
    # stack_name_base
    match = re.search(
        r'stack_name_base\s*[=:]\s*"?([^"\s]+)"?', content, re.MULTILINE
    )
    if match:
        config["stack_name_base"] = match.group(1).strip("\"'")

    # pattern (backend_pattern in tfvars, pattern in yaml)
    match = re.search(
        r'(?:backend_)?pattern\s*[=:]\s*"?([^"\s]+)"?', content
    )
    if match:
...
def main() -> int:
    ...
    if not stack_name:
        config = parse_config_yaml(tfvars_path)
    ...
    # Fetch stack outputs
    log_info(f"Fetching configuration from stack: {stack_name}")
    try:
