...
- [X] T071 [P] [US3] **Promoted in Phase 10d.** `model_agent.py` (~225 LOC) — calls the existing `generate_dbt_project` tool (the same one used by the single-source workflow's Step 4) to write a complete dbt project at `~/.dsa-hub/runs/<run_id>/dbt/<table>/`. Reads schema_agent's column list to compose the mart SQL; uses one staging model per source pull. Emits real `dbt_model` artifacts pointing at the generated `.sql` files + a `dbt_profile` artifact at the project's `profiles.yml`. Falls back to the deterministic stub when generate_dbt_project is unavailable or raises. **Smoke-tested end-to-end: real dbt project written, all 7 agents complete, run state=completed.**
